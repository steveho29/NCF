...
from Trainning.recommenders.datasets.pandas_df_utils import (
    has_columns,
    has_same_base_dtype,
    lru_cache_df,
)
...
def auc(
    ...
    Note:
        The calculate_metrics does not require a leave-one-out scenario.
...
def merge_ranking_true_pred(
    ...
    # Spark calculate_metrics metrics, where index of each recommended items (the indices are unique to items) is used
...
def map_at_k(
    ...
    The implementation of MAP is referenced from Spark MLlib calculate_metrics metrics.
    ...
    Note:
        1. The calculate_metrics function is named as 'MAP is at k' because the calculate_metrics class takes top k items for
...
Useful when we have to serialize calculate_metrics metric names
